...
// --- FLAT THEME DATA WITH FULL CODE CONTAINER STYLING ---
interface ThemeConfig {
  name: string;
  style: string;   // Outer wrapper gradient
  codeBg: string;  // Inner code window background
  headerBg: string;// Top Mac header background
...
function ExportHub() {
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute right-0 top-10 w-48 bg-zinc-900 border border-white/10 rounded-xl shadow-2xl p-1 z-50">
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 right-6 z-50 w-12 h-12 bg-black/60 backdrop-blur-xl border border-white/20 rounded-full flex items-center justify-center text-white hover:bg-black/80 transition-all shadow-2xl"
      >
      ...
      <div className={`fixed bottom-20 right-6 z-50 w-72 bg-black/70 backdrop-blur-2xl border border-white/20 rounded-2xl p-3 shadow-2xl transition-all duration-300 ${isOpen ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-4 scale-95 pointer-events-none"}`}>
        ...
        <div className="flex items-center gap-0.5 bg-zinc-800/50 rounded-lg p-0.5 mb-3">
          {paddings.map((p) => (
            <button key={p.value} onClick={() => setSettings({ ...settings, padding: p.value })} className={`flex-1 py-1.5 text-[11px] font-bold rounded-md transition-all ${settings.padding === p.value ? "bg-white text-black shadow-sm" : "text-zinc-400 hover:text-white"}`}>{p.label}</button>
          ))}
...
function hello() {
  ...
  // Return full theme config safely
  const getActiveTheme = () => {
    return ALL_THEMES[settings.bg] || ALL_THEMES.midnight;
  };
    ...
    <main className="flex flex-col lg:flex-row h-screen w-full bg-black text-white relative">
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-white/10 px-4 sm:px-6">
        ...
        <div className={`text-xs font-medium shrink-0 ml-auto sm:ml-0 ${generationsLeft <= 1 ? "text-red-500" : "text-zinc-500"}`}>
          {isPro ? "Pro ✨" : `${generationsLeft} free left`}
        </div>
      </header>
      ...
      <div className="flex flex-col lg:flex-row min-h-0 flex-1 overflow-hidden">
        <div className="flex w-full lg:w-1/2 flex-col overflow-hidden border-b lg:border-b-0 lg:border-r border-white/10 p-4 sm:p-6">
          ...
          <Button onClick={handleGenerate} disabled={loading} className="mt-3 h-11 w-full shrink-0 bg-white text-black font-semibold text-sm hover:bg-zinc-200 disabled:opacity-50">
        ...
        <div className="flex w-full lg:w-1/2 flex-col overflow-hidden p-4 sm:p-6 min-h-0 relative">
          ...
          <Tabs defaultValue="image" className="flex min-h-0 flex-1 flex-col">
              ...
              <div className="flex flex-1 min-h-0 items-center justify-center overflow-hidden p-4">
                ...
                <div className={`w-full max-w-2xl h-full rounded-xl transition-all duration-300 flex flex-col ${getActiveTheme().style} ${settings.padding}`}>
                  ...
                  <div className={`flex-shrink-0 flex items-center justify-between px-4 py-3 rounded-t-lg relative h-10 backdrop-blur-sm ${getActiveTheme().headerBg} border-b border-white/10`}>
                    ...
                    {!isPro && (
                      <div className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex gap-1 text-sm font-mono ${getActiveTheme().text} opacity-80`}>
                        <span>code</span>
                  ...
                  {/* CODE AREA (Theme adapted) */}
                  <div className={`flex-grow overflow-y-auto rounded-b-lg backdrop-blur-sm ${getActiveTheme().codeBg} font-mono text-sm`}>
                    {code.split('\n').map((line, i) => (
                      <div key={i} className={`flex hover:bg-white/5 -mx-6 px-6 ${getActiveTheme().text}`}>
                        {settings.showLines && (
                          <span className={`w-8 text-right pr-4 select-none text-xs shrink-0 leading-6 opacity-40`}>
                            {i + 1}
                          </span>
                        )}
            ...
            <TabsContent value="platform" className="flex flex-1 flex-col gap-4 rounded-lg border border-white/10 bg-zinc-950/50 p-4 mt-0 min-h-0 overflow-auto">
      ...
      {showPaywall && (
        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-zinc-950 border border-white/10 rounded-xl p-8 max-w-sm text-center">
            <h3 className="text-xl font-bold mb-2">Premium Features Locked 🛑</h3>
            <p className="text-sm text-zinc-400 mb-6">Upgrade to Pro to unlock premium themes, hide the logo, and remove limits!</p>
            <Button onClick={() => setShowPaywall(false)} className="w-full bg-white text-black hover:bg-zinc-200">Maybe Later</Button>
          </div>
        </div>
      )}
    </main>
