...
export default function ToolUI({ initialGenerationsLeft, userId }: { initialGenerationsLeft: number, userId: string }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({ code, language, platform, userId }),
      });
  ...
  return (
    <main className="flex flex-col lg:flex-row h-screen w-full bg-black text-white relative">
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-white/10 px-4 sm:px-6">
        ...
        <div className={`text-xs font-medium shrink-0 ml-auto sm:ml-0 ${generationsLeft <= 1 ? "text-red-500" : "text-zinc-500"}`}>{generationsLeft} free left</div>
      </header>
      ...
      <div className="flex flex-col lg:flex-row min-h-0 flex-1 overflow-hidden">
        <div className="flex w-full lg:w-1/2 flex-col overflow-hidden border-b lg:border-b-0 lg:border-r border-white/10 p-4 sm:p-6">
          ...
          <Textarea value={code} onChange={handleCodeChange} placeholder="Paste your code here..." className="flex-1 resize-none rounded-lg border-white/10 bg-zinc-950 p-4 font-mono text-sm focus-visible:ring-1 focus-visible:ring-white/20" />
          <div className="mt-2 flex items-center justify-between h-4">
            <span className="text-xs text-zinc-700">{lineCount} lines</span>
            {lineCount > 45 && <span className="text-xs text-yellow-500 font-medium">⚠️ Long code may get cropped on X/LinkedIn</span>}
          </div>
          ...
          <Tabs defaultValue="image" className="flex min-h-0 flex-1 flex-col">
            ...
            <TabsContent value="image" className="flex flex-1 flex-col rounded-lg border border-dashed border-white/10 bg-zinc-950/50 mt-0 p-4 min-h-0 overflow-hidden">
              <div className="flex flex-1 min-h-0 items-center justify-center overflow-auto p-2">
                {image ? <img src={image} alt="Generated Code" className="max-w-full max-h-full object-contain rounded-md shadow-2xl" /> : <p className="text-xs text-zinc-600">Image preview will appear here</p>}
              </div>
              {image && (
                <div className="flex-shrink-0 mt-4 pt-4 border-t border-white/5">
                  <Button onClick={handleDownload} className="w-full bg-white text-black font-semibold text-sm hover:bg-zinc-200">Download PNG 📸</Button>
                </div>
              )}
            </TabsContent>
      ...
      {showPaywall && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-zinc-950 border border-white/10 rounded-xl p-8 max-w-sm text-center">
            <h3 className="text-xl font-bold mb-2">Out of free generations! 🛑</h3>
            ...
            <Button onClick={() => setShowPaywall(false)} className="w-full bg-white text-black hover:bg-zinc-200">Got it</Button>
          </div>
        </div>
      )}
    </main>
